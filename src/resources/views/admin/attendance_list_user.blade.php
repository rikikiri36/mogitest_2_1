...
<form action="/admin/attendance/staff/exportcsv" method="POST">
  @csrf
  <h1 class="title">{{ $user->name ?? '' }}さんの勤怠</h1>
  <input type="hidden" name="user_name" value="{{ $user->name }}">
  <div class="select-month">
    <a class="prev-month" href="/admin/attendance/staff/{{ $user->id ?? '' }}?action=prev&Ym={{ $Ym ?? '' }}">
    ...
    <a class="next-month" href="/admin/attendance/staff/{{ $user->id }}?action=next&Ym={{ $Ym }}">
      次月
  ...
  <table class="attendance-table">
    ...
    @foreach($calendarData as $day)
      <tr>
        ...
        <td>
          @if ($day['detail_id'])
            <a href="/admin/attendance/{{ $day['detail_id'] }}" class="detail__link">詳細</a></td>
          @endif
        </tr>
    @endforeach
    </tbody>
  ...
  <div class="btn_area">
    <button type="submit" class="edit__button btn">CSV出力</button>
    <input type="hidden" name="results" value="{{ base64_encode(serialize($calendarData)) }}">
  </div>
</form>
</div>
@endsection('content')